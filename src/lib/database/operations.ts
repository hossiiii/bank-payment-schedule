...
export class TransactionOperations {
  ...
  async create(transactionData: TransactionInput): Promise<Transaction> {
    return withRetry(async () => {
      ...
      const transaction: Transaction = {
        ...
        storeName: validatedTransaction.storeName || '',
        usage: validatedTransaction.usage || '',
        amount: validatedTransaction.amount,
        paymentType: validatedTransaction.paymentType,
        cardId: validatedTransaction.cardId || '',
        bankId: validatedTransaction.bankId || '',
        scheduledPayDate: validatedTransaction.scheduledPayDate,
        isScheduleEditable: validatedTransaction.isScheduleEditable || false,
        memo: validatedTransaction.memo || '',
    ...
    return withRetry(async () => {
      ...
      const items: ScheduleItem[] = transactions.map(tx => {
        ...
        return {
          ...
          cardName: cardName || '',
          isScheduleEditable: tx.isScheduleEditable || false
        };
      });
  ...
  async updateScheduledPayDatesForCard(_cardId: string, newDates: Record<string, number>): Promise<void> {
    return withRetry(async () => {
      ...
      for (const [transactionId, scheduledPayDate] of Object.entries(newDates)) {
