...
export class BankOperations {
  ...
  async create(bankData: BankInput): Promise<Bank> {
    return withRetry(async () => {
      ...
      const bank: Bank = {
        id: crypto.randomUUID(), // Generate UUID directly
        name: validatedData.name,
        ...(validatedData.memo && { memo: validatedData.memo }),
        createdAt: Date.now()
      };
      
      await this.db.banks.add(bank);
      return bank;
    });
  }
  ...
  async delete(id: string): Promise<void> {
    return withRetry(async () => {
      ...
      const bank = await this.db.banks.get(id);
      if (!bank) {
        throw new ValidationError(`Bank with ID "${id}" not found`);
      }
  ...
  async create(cardData: CardInput): Promise<Card> {
    return withRetry(async () => {
      ...
      const card: Card = {
        ...
        paymentMonthShift: validatedData.paymentMonthShift,
        adjustWeekend: validatedData.adjustWeekend,
        ...(validatedData.memo && { memo: validatedData.memo }),
        createdAt: Date.now()
      };
...
export class TransactionOperations {
  ...
  async create(transactionData: TransactionInput): Promise<Transaction> {
    return withRetry(async () => {
      ...
      const transaction: Transaction = {
        ...
        paymentType: validatedTransaction.paymentType,
        scheduledPayDate: validatedTransaction.scheduledPayDate,
        createdAt: validatedTransaction.createdAt,
        ...(validatedTransaction.storeName && { storeName: validatedTransaction.storeName }),
        ...(validatedTransaction.usage && { usage: validatedTransaction.usage }),
        ...(validatedTransaction.cardId && { cardId: validatedTransaction.cardId }),
        ...(validatedTransaction.bankId && { bankId: validatedTransaction.bankId }),
        ...(validatedTransaction.isScheduleEditable && { isScheduleEditable: validatedTransaction.isScheduleEditable }),
        ...(validatedTransaction.memo && { memo: validatedTransaction.memo })
      };
   ...
   */
  async updateScheduledPayDatesForCard(newDates: Record<string, number>): Promise<void> {
    return withRetry(async () => {
      const updates = Object.entries(newDates).map(async ([transactionId, scheduledPayDate]) => {
        await this.db.transactions.update(transactionId, { scheduledPayDate });
      });
